						...
						devices
								.forEach(function(device) {
									if (device.kind === 'videoinput') {
										if (device.label.toLowerCase().search(
												"back") > -1)
											options = {
												'deviceId' : {
...
function setwebcam2(options){ 
	...
	p.then(success, error); // setTimeout(captureToCanvas,
}
																															// 500);
...
function load(){ 
	alert(1);
	initCanvas(800,600); qrcode.callback = read; setwebcam(); 
} 
function read(a){ 
	
	
	document.getElementById("result").innerHTML = a.toString(); 
	console.log("qrcode info:"+a);
	// 输出扫描后的信息
}
																																																																															// }
																													// }
